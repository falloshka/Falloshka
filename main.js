...
async function generateTarotReading() {
  ...
  const past =
    selectedCards[0]?.name[lang] || selectedCards[0]?.name.en;
  const present =
    selectedCards[1]?.name[lang] || selectedCards[1]?.name.en;
  const future =
    selectedCards[2]?.name[lang] || selectedCards[2]?.name.en;

  // Kullanıcıya ara mesaj
  writeNarrative(translations[lang]?.tarotLoading || "");

  try {
    const res = await fetch(
      "https://falloshka-astro-backend.vercel.app/api/tarot",
      {
        method: "POST",
        headers: {
    ...
    if (!res.ok) {
      throw new Error("Tarot API error");
    }
